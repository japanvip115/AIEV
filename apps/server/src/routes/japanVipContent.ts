...
function imageWritingContext(project: JapanVipContentProject): string {
  ...
  return [
    ...
    ...approved.map((image) => {
      const format = resolveImageFormat(image);
      return `- role=${image.role}; khung=${format.box.maxWidth}×${format.box.maxHeight}px; section=${image.intendedSection || "tự ghép theo ngữ cảnh"}; group=${image.featureGroup || "none"}; alt=${image.altText}; caption=${image.caption}; url=${image.url}`;
    }),
...
async function runHermesReview(project: JapanVipContentProject) {
  ...
  const prompt = [
    ...
    "Không đề xuất chèn nhãn [CẦN KIỂM CHỨNG] hoặc viết về dữ liệu đang thiếu. Chi tiết không có bằng chứng phải được bỏ khỏi bài; thông tin an toàn bắt buộc được kiểm tra riêng trong fact sheet.",
  ...
  const review = {
    ...
    suggestedRules: textList(parsed.suggestedRules, 8).filter((rule) => !/\[CẦN KIỂM CHỨNG\]/i.test(rule)),
    criteria,
